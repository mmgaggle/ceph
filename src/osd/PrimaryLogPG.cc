...
bool PrimaryLogPG::start_oob_delivery(OpContext *ctx, MOSDOpReply *reply)
{
  ...
  for (size_t i = 0; i < rops.size(); i++) {
    ...
    if (!plan_op_oob(ctx, rops[i], d, req.plan, req.data,
		     state->sparse_extents[i], store_ofs)) {
      continue;
    }
    if (d.flags & ceph::rdma::delivery_t::FLAG_CRC64NVME) {
      // what the store can fold from metadata is settled here, under
      // the PG lock; the delivery thread hashes only the rest
      req.known_crc64.reserve(req.plan.size());
      for (const auto& t : req.plan) {
	req.known_crc64.push_back(oob_range_crc64_from_store(
	  m->get_hobj(), store_ofs, t.local_ofs, t.len));
      }
    }
