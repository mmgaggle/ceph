...
private:
  ...
  struct staged_payload {
    struct segment {
      ...
      uint64_t handle_ofs;  ///< where it starts in handle's registration
      bool cached;          ///< handle belongs to the pool-buffer cache
    };
  ...
  /// the registration of one of the bdev's recycled huge-page read
  /// buffers (bdev_read_preallocated_huge_buffers), made on first sight
  /// and kept: those regions stay mapped for the life of the process
  struct rdma_buffer* pool_buffer_handle(char* base, size_t len);
  ...
  ceph::shared_mutex m_pool_handles_lock =
    ceph::make_shared_mutex("OSDCuObj::pool_handles");
  std::map<const char*, struct rdma_buffer*> m_pool_handles;
