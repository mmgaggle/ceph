...
protected:
  ...
  /**
   * deliver_oob without holding the op worker or the PG lock for the
   * transfer: hands each plan to the cuObject delivery threads, which
   * send the finished reply once the last one completes (stripping the
   * ops that were pushed, leaving the rest inline). Returns false if
   * nothing was queued, in which case the caller still owns reply.
   */
  bool start_oob_delivery(OpContext *ctx, MOSDOpReply *reply);
#endif
